      ...
      {TAG_SWATCHES.map((s) => (
        <button
          ...
          className={`h-6 w-6 rounded-full border-2 transition-transform duration-[120ms] hover:scale-110 ${
            value === s.id ? 'scale-100 border-accent' : 'border-transparent'
          }`}
        />
      ))}
    </div>
...
function SystemTagGroup({ label, hint, items }: { label: string; hint: string; items: UserTag[] }) {
        ...
        {items.map((t) => {
          ...
          return (
            ...
            >
              ...
              <span className="ml-auto font-mono text-[0.6875rem] text-neutral-500">
              ...
              return (
                ...
                >
                  <div
                    ...
                    style={{
                      background: hex,
                      boxShadow: '0 0 0 1px rgb(var(--fg-inverse) / 0.06) inset',
                    }}
                  />
                  <div>
                    <div className="font-sans text-sm font-medium text-neutral-900 dark:text-neutral-100">
                      {t.name}
                    </div>
                    <div className="font-mono text-[0.6875rem] text-neutral-400 dark:text-neutral-500">
                      #{t.id}
                    </div>
                  </div>
                  <div>
                  ...
                  {confirming && (
                    <div className="col-span-5 flex items-center gap-3 pt-2 font-sans text-sm text-neutral-500 dark:text-neutral-400">
                      <span>
                        Delete{' '}
                        <strong className="text-neutral-900 dark:text-neutral-100">{t.name}</strong>
                        ?
                        {t.item_count > 0 && (
                          <>
                            {' '}
