...
// DOS-only "installed" flag: whether game files are already copied onto the
// drive, letting Peach 1UP skip the copy step and boot directly. Gated
// behind a confirm dialog since flipping it wrong causes either a
// stale-copy boot or an unwanted re-copy on next launch.
export function useInstalledToggle({ collectionId, detailQueryKey }: UseInstalledToggleOptions) {
