...
describe('useConfirmToken', () => {
  ...
  it('issue → consume → DELETE fires with token in query string', async () => {
    ...
    expect(mockApiFetch).toHaveBeenLastCalledWith('/api/v1/library/1?confirmation_token=tok123', {
      method: 'DELETE',
    });
  });
  ...
  it('rapid re-issue does not reject', async () => {
    ...
    expect(
      mockApiFetch.mock.calls.every(([, init]) => (init as RequestInit)?.method === 'POST'),
    ).toBe(true);
  });
});
