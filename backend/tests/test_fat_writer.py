...
def _make_image(tmp_path, size_mb=10):
    from backend.service.utils.fat_writer import format_fat16
...
class TestWriteReadRoundTrip:
    def test_write_and_read_back_same_bytes(self, tmp_path):
        from backend.service.utils.fat_writer import write_file_to_image, read_file_from_image
...
class TestLongFilenames:
    ...
    def test_truncated_names_collide(self, tmp_path):
        from backend.service.utils.fat_writer import write_file_to_image
        img = _make_image(tmp_path)
...
class TestReadFileErrors:
    def test_missing_file_raises_runtime_error(self, tmp_path):
        from backend.service.utils.fat_writer import read_file_from_image
        img = _make_image(tmp_path)
