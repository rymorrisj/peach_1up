...
def _with_stats(profile: Profile, db: Session) -> ProfileRead:
    (item_count,) = db.query(func.count(GameItemBundle.id)).filter(GameItemBundle.profile_id == profile.id).one()
...
def _with_stats_bulk(profiles: list[Profile], db: Session) -> list[ProfileRead]:
    ...
    item_counts = dict(
        db.query(GameItemBundle.profile_id, func.count(GameItemBundle.id))
        .filter(GameItemBundle.profile_id.in_(ids))
        .group_by(GameItemBundle.profile_id)
        .all()
    )
    launch_stats = {
...
def create_profile(body: ProfileCreate, db: Session = Depends(get_db), _: User = require_permission("can_manage_game")):
...
def get_profile_items(
    ...
    if not profile:
        raise HTTPException(status_code=404, detail="Profile not found.")
    q = db.query(GameItemBundle).filter(GameItemBundle.profile_id == profile.id)
    total = q.count()
...
def update_profile(slug: str, body: ProfileUpdate, db: Session = Depends(get_db), _: User = require_permission("can_manage_game")):
    profile = db.query(Profile).filter(Profile.slug == slug).first()
    if not profile:
        raise HTTPException(status_code=404, detail="Profile not found.")
...
def delete_profile(slug: str, db: Session = Depends(get_db), _: User = require_permission("can_manage_game")):
    profile = db.query(Profile).filter(Profile.slug == slug).first()
    if not profile:
        raise HTTPException(status_code=404, detail="Profile not found.")
