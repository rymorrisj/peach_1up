...
def health_recompute_all(db: Session = Depends(get_db), _: UserItem = require_permission("can_manage_environment")):
    return env_svc.batch_health_check(db)


@router.get("/health/storage-stats", response_model=StorageStats)
def health_storage_stats(db: Session = Depends(get_db), _: UserItem = require_permission("can_manage_environment")):
