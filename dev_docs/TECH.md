...
`smart_media_detector` is the self-contained media detection package that identifies platform and era from a disk image or directory path. It lives at `backend/service/utils/smart_media_detector/` and is designed to be vendored into its own standalone repository after the Beta ships. The package itself now has zero `backend.*` imports: `detector.py` and `directory_detect.py` previously imported `backend.core.logger` for their module loggers and now construct them with stdlib `logging.getLogger(__name__)` instead. One known external dependency remains: `iso_detect.py` imports a sibling module, `backend/service/utils/xbox_image.py`, that lives outside the package folder and is also used by `xemu.py`, `coordinator.py`, and `extract_xiso.py` elsewhere in the backend, so it was left in place rather than moved, to avoid breaking those other call sites. The package no longer imports `backend.service.backends.rpcs3` either. `is_disc_format_folder`/`find_eboot` moved into `directory_detect.py` as part of the MediaTarget refactor (Step 3), closing the one backend-into-detector dependency this package had. See the package's own README.md for the full standalone-package-readiness rundown, including the extraction readiness checklist and known limitations.
