...
per-emulator descriptors in `config/emulators/`; when the two
disagree, the TOML wins and this file is the bug.

Related: [Technology Stack](./tech-stack.mdx) (stack) · [Security Architecture](./security.mdx) (isolation policy) ·
[Windows Sandboxing](./windows-sandboxing.mdx) (resource caps).
...
[Windows Sandboxing](./windows-sandboxing.mdx) § DOSBox-X specifics.
...
|---|---|---|
| AppContainer not supported | critical | Heavily JIT-based recompiler (Cell PPU/SPU and RSX) relying on runtime code generation and low-level syscalls that AppContainer's restricted token blocks. Job Object isolation only. Note that `container_permanently_excluded = false` in the TOML contradicts this entry; see the open items in [the changelog](https://github.com/rymorrisj/peach_1up/blob/main/CHANGELOG.md). |
...
Detection lives in
`detection/xbox_image.py`, with the
...
The root `LICENSE` is GPL-2.0-or-later.
