...
The library path settings panel is not available in the UI for this build, and settings are DB-backed now (`app_settings` table) — the previous workaround of hand-editing `config/settings.yaml` no longer applies. There is currently no user-facing way to change LIBRARY_PATH, PROFILES_PATH, or ROMS_PATH. The 86Box ROM path is the most common case — if 86Box fails to launch with a ROM path error, place the ROM pack at the default location, `{install directory}/library/system/roms/86box`.
