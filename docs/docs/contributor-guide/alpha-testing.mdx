...
A missing ROM pack or an immediate post-launch crash now fails the launch attempt outright with a visible error message (HTTP 500) rather than appearing to launch successfully. If you see this, check Settings → Emulators → 86Box for a ROM path warning. If the path is wrong, edit `config/settings.yaml` and set `ROMS_PATH` to the correct directory, then restart the backend from the system tray.
