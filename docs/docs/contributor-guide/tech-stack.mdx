...
PyInstaller compiles the Python backend to a standalone executable. React builds to static files served by FastAPI. pystray provides a system tray icon. Windows installer via NSIS/WiX, Linux via deb/AppImage.
...
1. **Hash lookup** — full-file SHA-1 (with MD5/CRC32 fallback) compared against a bundled `hash_index.json` built from No-Intro/Redump DAT files. Returns the highest-confidence result possible; exits immediately on a hit.
