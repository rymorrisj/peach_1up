...
The Library Paths settings panel has been removed from the UI for now. LIBRARY_PATH, PROFILES_PATH, and ROMS_PATH can no longer be set through the frontend. Users with non-standard installs (library on a secondary drive, NAS share, or external volume) must edit config/settings.yaml directly to reconfigure these paths.
