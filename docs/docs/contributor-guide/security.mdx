...
Companion documents: [Auth Reference](./auth.mdx) (auth flows and permission model) ·
[Windows Sandboxing](./windows-sandboxing.mdx) (Job Object and resource caps) ·
[the changelog](https://github.com/rymorrisj/peach_1up/blob/main/CHANGELOG.md) (decision history).
...
`models/user.py`; keep this in step with it. See
[Auth Reference](./auth.mdx) for per-endpoint detail.
...
- The **owner's** PIN can never be reset through this endpoint by anyone. Owner-target
  requests get 403. Recovery is `scripts/setup_admin_user.py`
  only.
...
- The scale is freetext on `GameItemBundle.content_rating` and
  ...
  derived in `dependencies.py`. No write path exists for
...
- **Emulator binary paths are never taken from request input**, query parameters, or
  ...
  (`emulator_catalog.py`) resolves exactly
...
3. Tokens expire after **60 seconds** (`TOKEN_TTL` in
   `confirmation_tokens.py`). Expired
   tokens are rejected.
...
- Third-party API credentials live in `.env` only, via
  `env_secrets.py`. They are never committed,
...
`core/database.py`, and separately in
`scripts/setup_admin_user.py`, which builds its own
standalone engine.
...
- Every launch is assigned to a fresh Job Object with kill-on-close, a per-era CPU rate
  ...
  **Every descriptor in `config/emulators/` currently sets both to
  `false`**, so both caps are enforced for every emulator as shipped and kill-on-close is
  not the only control in force. See [Windows Sandboxing](./windows-sandboxing.mdx) for the
  per-era numbers.
...
- Container-enable resolution and media-broker config are single implementations in
  ...
  see [the changelog](https://github.com/rymorrisj/peach_1up/blob/main/CHANGELOG.md) open items.
...
Implementation lives in the vendored `wincage` package:
...
`formatscout` package, with the Peach-specific resolvers
in `detection/`.
...
[the changelog](https://github.com/rymorrisj/peach_1up/blob/main/CHANGELOG.md) in the same change that flips the TOML value, so this table
...
The IP-keyed limiter (`core/rate_limit.py`) stores counts
...
The bundled index (`formatscout/hashing/hash_index.json` inside the installed package)
...
`environments/environments.py` confirms
...
`test.yml` runs on every push and PR to `main`: it
...
exe serves `/api/openapi.json`. `codeql.yml` provides
static analysis.
...
Earlier revisions of this document and of [Auth Reference](./auth.mdx) described a recovery key
