...
- Every emulator launch is assigned to a fresh Job Object with kill-on-close,
  ...
  every emulator except RPCS3 sets this today.
...
- AppContainer is an additional isolation layer applied on top of Job Objects when
  ...
  Mesen, PCSX2, Project64, Xenia, and xemu all have `container_enabled = true`. 86Box
  and RPCS3 remain disabled, RPCS3 due to documented JIT/TCG incompatibility
  (`container_permanently_excluded`), 86Box pending further testing.
...
(86Box, DuckStation, PCSX2) or the managed-runtime Mesen (.NET/Avalonia).
Qt's security initialisation triggers STATUS_STACK_BUFFER_OVERRUN
(0xC0000409) when this limit is applied after process creation. Mesen
pre-allocates heap at startup via its managed runtime and exhibits the
same failure. Kill-on-close and CPU rate control still apply. The
skip_memory_limit flag in config/emulators/ controls this per emulator.

xemu also pre-allocates a JIT heap at startup but is not exempt: its
memory cap is enforced (skip_memory_limit = false) as long as the xbox
era's memory_limit_mb stays above 2048 MB, which avoids the failure mode
above.
...
skip_cpu_limit = true in the emulator descriptor, every emulator except RPCS3 (the
only one that keeps the enforced CPU cap) sets this today; the launcher honours it via
...
removed from scope entirely. Process isolation is Windows Job Objects only; there is
no Linux code path to secure.
