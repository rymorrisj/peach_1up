...
- Rating scale is freetext on `SoftwareCollection.content_rating`. Recommended: ESRB (E, E10+, T, M, AO) or
  PEGI (3, 7, 12, 16, 18). Ordinal comparison map configured via the `rating_ordinals` key in `app_settings`.
...
- Emulator binary paths are **never taken from request input**, query parameters, or
  ...
  `app_settings` DB table, (2) bundled project `emulators/{slug}/` directory,
...
- Emulator binary paths are never derived from request input. Paths resolve through
  three tiers: `app_settings` DB user override, bundled `emulators/{slug}/` project
...
DB-backed in the `app_settings` table. LIBRARY_PATH, SOFTWARE_PATH, PROFILES_PATH, and
...
writes directly to `app_settings`, but it requires a `can_manage_settings` session — in
