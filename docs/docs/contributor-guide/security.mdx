...
| `can_edit_environments`  | Register or modify Environments (Windows OS install workspaces; was `can_edit_platforms`) |
| `can_manage_software`    | Add, edit, delete, or scan/import software collections and items, and create/modify/delete launch Profiles. Gates `POST /api/v1/software/scan`. Was `can_edit_library` |
...
| `can_manage_profiles`    | Orphaned flag: still present and surfaced in the Users UI, but no route enforces it any more (Profile CRUD moved to `can_manage_software`). Slated for cleanup |
...
local `setup_admin_user.py` path only). A non-locked sub-account can have its PIN
reset by the owner or any admin, and a `can_manage_users` holder can reset its own
PIN. A locked account can only be reset by an owner or admin (the self-service
path is blocked while locked).
...
writes directly to `app_settings`, but it requires a `can_edit_settings` session — in
