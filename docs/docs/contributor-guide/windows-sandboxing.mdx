...
the emulator descriptor. Eight emulators have it enabled (DOSBox-X, DuckStation,
Flycast, Mesen, PCSX2, Project64, Xenia, xemu); 86Box and RPCS3 remain Job-Object-only
(see the [Security Architecture](./security.mdx) page for the reasons). A dedicated
...
- It does not apply on Linux. Linux support is out of scope.
...
`container_enabled = true` in `config/emulators/xemu.toml`: AppContainer isolation is
enabled for xemu. It was previously suspected that xemu's QEMU TCG backend was
incompatible with AppContainer's `DeviceIoControl` restrictions, but the actual cause
of earlier launch failures was memory/CPU era limits too small for xemu's pre-allocated
JIT heap. With adequate limits, xemu runs correctly under AppContainer.

Both `skip_memory_limit = false` and `skip_cpu_limit = false` apply to xemu, matching
most other emulators: the Job Object memory and CPU caps are enforced using the xbox
era's limits (`config/eras.yaml`), which must stay above 2048 MB for the memory cap or
xemu exits immediately with a paging file error.
